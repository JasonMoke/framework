...
import com.core.helper.session.SessionHelper;
...
//		boolean rememberMe = isRememberMe(servletRequest);
		...
		return new AuthenticationToken(username, password, host);
//		return new AuthenticationToken(username, password, captchaId, captcha, rememberMe, host);
	...
	protected boolean onAccessDenied(ServletRequest servletRequest, ServletResponse servletResponse) throws Exception {
		...
		if(session.getAttribute(Global.SESSION_FORCE_LOGOUT_KEY)!=null){
			HttpServletResponse httpRes=(HttpServletResponse)response;
			String contextPath = ((HttpServletRequest) request).getContextPath();
			try {  
				SessionHelper.logout(request.getSession());
	            getSubject(request, response).logout();//强制退出
	        } catch (Exception e) {
	        	/*ignore exception*/
	        }
			httpRes.sendRedirect(contextPath + StringUtils.defaultIfEmpty("/login.jsp", "/"));
			return false;  
		}
	...
	protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {  
        ...
        return session.getAttribute(Global.SESSION_FORCE_LOGOUT_KEY) == null;  
    }  
